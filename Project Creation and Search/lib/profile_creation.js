...
const generateRandomID = async db => {
  ...
  await get(child(ref(db), "lfg-test/users")).then(async snapshot => {
    if (snapshot.exists()) for (let id in snapshot.val()) {
...
const init = () => {
  ...
  submit.onclick = () => {
    ...
    writeProfileData(uname.value, fname.value, lname.value, bio.value, role.value, pronouns, pIntInput.value, rIntInput.value, availInput.value, linkHolder, skillHolder);
  };
  skillAdd.onclick = () => {
